...
test('list builder exposes audited recipes plus seven advanced matchup recipes', () => {
  assert.equal(PRIMARY_LIST_BUILDER_PRESETS.length, 6)
  assert.equal(MORE_LIST_BUILDER_PRESETS.length, 4)
  assert.equal(ADVANCED_LIST_BUILDER_PRESETS.length, 7)
  ...
  assert.deepEqual(ADVANCED_LIST_BUILDER_PRESETS.map((preset) => preset.id), [
    'pitch-type-punish', 'recent-pitcher-leak', 'contact-collision', 'zone-contact-focus', 'pitcher-contact-leak', 'low-k-power', 'top-four-power',
  ])
})
...
test('advanced recipes disclose the exact historical features needed for readiness', () => {
  ...
  assert.equal(ADVANCED_LIST_BUILDER_PRESETS.find((preset) => preset.id === 'pitcher-contact-leak').readiness.fallbackStatus, 'collecting')
})
