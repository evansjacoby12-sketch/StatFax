...
test('Games workspace receives and labels advisory game forecasts and validation', async () => {
  ...
  assert.match(games, /Model call/i)
  assert.match(games, /\{winnerAbbr\} to win/i)
  assert.match(games, /Total call/i)
  assert.match(games, /No total posted/i)
  assert.match(games, /Market line unavailable/i)
  ...
  assert.match(css, /\.game-forecast-call-item/)
