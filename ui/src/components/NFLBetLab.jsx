...
function ComboGrid({ combos, slip, onAddCombo, disabled = false }) {
  ...
  return <div className="nfl-combo-grid">{combos.map((combo, index) => {
    ...
    return <article className="nfl-combo-card" key={combo.id} style={{ '--nfl-combo-grade': color }}>
      ...
      <footer><button type="button" className={isAdded ? 'active' : ''} disabled={disabled} onClick={() => onAddCombo(combo)}><Icon name={disabled ? 'Lock' : isAdded ? 'Check' : 'Plus'} size={14} />{disabled ? 'Grades on hold' : isAdded ? 'Combo added' : `Add all ${combo.legs.length} legs`}</button></footer>
    </article>
  })}</div>
}

function ComboExplorer({ snapshot, slip, onAddCombo, scope, legCount, setLegCount, strategy, setStrategy, minGrade, setMinGrade, disabled = false }) {
  ...
  return <section className="nfl-combo-explorer" aria-label={scope === 'same-game' ? 'NFL same-game combinations' : 'NFL combination explorer'}>
    ...
    <ComboGrid combos={combos} slip={slip} onAddCombo={onAddCombo} disabled={disabled} />
  </section>
}

function CustomBuilder({ slipLegs, onToggleLeg, onClearSlip, onSaveTicket, disabled = false }) {
  ...
  return <section className="nfl-custom-builder" aria-labelledby="nfl-custom-builder-title">
    ...
    <footer><button type="button" onClick={onClearSlip} disabled={!slipLegs.length}>Clear</button><button type="button" className="primary" onClick={onSaveTicket} disabled={disabled || !slipLegs.length}><Icon name={disabled ? 'Lock' : 'Bookmark'} size={14} />{disabled ? 'Grades on hold' : 'Track ticket'}</button></footer>
  </section>
}
...
export default function NFLBetLab({ snapshot, slip, slipLegs, tab, onTabChange, onAddCombo, onToggleLeg, onClearSlip, onSaveTicket }) {
  ...
  const disabled = snapshot.modelReadiness?.gradesEnabled === false
  ...
  >
    ...
    {disabled && <div className="nfl-lab-readiness-hold"><Icon name="Lock" size={14} /><span><b>Bet Lab actions are paused.</b> Research remains available while the engine clears its blocking readiness checks.</span></div>}
    {tab === 'explore' && <ComboExplorer snapshot={snapshot} slip={slip} onAddCombo={onAddCombo} scope="all" legCount={legCount} setLegCount={setLegCount} strategy={strategy} setStrategy={setStrategy} minGrade={minGrade} setMinGrade={setMinGrade} disabled={disabled} />}
    {tab === 'builder' && <CustomBuilder slipLegs={slipLegs} onToggleLeg={onToggleLeg} onClearSlip={onClearSlip} onSaveTicket={onSaveTicket} disabled={disabled} />}
    {tab === 'same-game' && <ComboExplorer snapshot={snapshot} slip={slip} onAddCombo={onAddCombo} scope="same-game" legCount={legCount} setLegCount={setLegCount} strategy={strategy} setStrategy={setStrategy} minGrade={minGrade} setMinGrade={setMinGrade} disabled={disabled} />}
  </WorkspaceShell>
}
