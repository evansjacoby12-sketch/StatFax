...
const RECENT_DAYS = 7
const STRAT_LABEL = { core: 'Core Pair', top: 'Top Picks', mix: 'Best Mix', stack: 'Signal Stack', hot: 'Hot Hand', power: 'Power Bats', matchup: 'Soft Matchup', park: 'Park & Air', precision: 'Precision', value: 'Value', edge: 'Edge Stack' }
...
export default function CombosView({ batters, onSelect, favorConsistency = false, initialSection = 'live' }) {
  ...
  const settled = (() => {
    if (!log) return null
    // Prefer fullByDate — it's the current writer. The legacy byDate key can
    // hold an EARLIER run's combos for the same day (observed 07-03: byDate
    // lacked the precision combos fullByDate carried), so it must not clobber.
    const comboByDate = { ...(log.combos?.byDate || {}), ...(log.combos?.fullByDate || {}) }
