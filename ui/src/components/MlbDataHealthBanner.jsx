...
export default function MlbDataHealthBanner({ health }) {
  ...
  const summary = health.status === 'critical'
    ...
    : `${warnings} data check${warnings === 1 ? '' : 's'}${affectedGames ? ` across ${affectedGames} game${affectedGames === 1 ? '' : 's'}` : ''}${affectedBatters ? ` · ${affectedBatters} hitter${affectedBatters === 1 ? '' : 's'} marked for review` : ''}. Projections were not changed by ${warnings === 1 ? 'this check' : 'these checks'}.`
