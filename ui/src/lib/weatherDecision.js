...
export function classifyWeatherGame(g = {}) {
  ...
  if (g.closed) return { key: 'dome', label: isDome ? 'Dome' : 'Roof closed', icon: 'House', note: 'No wind effect', favorable: false }
  if (precip >= 50) return { key: 'rain', label: 'Rain watch', icon: 'CloudRain', note: `${Math.round(precip)}% precipitation`, favorable: false }
  ...
  if (parkBias === 'boost' && airBias === 'boost') {
    return { key: 'favorable', label: 'Favorable', icon: 'TrendingUp', note: 'Park + air aligned', favorable: true }
  }
  if (parkBias === 'drag' && airBias === 'drag') {
    return { key: 'suppressed', label: 'Suppressed', icon: 'TrendingDown', note: `${parkText(g.parkHR)} + wind in`, favorable: false }
  }
  if (parkBias === 'boost' && (airBias === 'neutral' || airBias === 'unknown')) {
    return { key: 'park-boost', label: 'Park boost', icon: 'Gauge', note: parkText(g.parkHR), favorable: true }
  }
  if (parkBias === 'drag' && (airBias === 'neutral' || airBias === 'unknown')) {
    return { key: 'suppressed', label: 'Suppressed', icon: 'TrendingDown', note: parkText(g.parkHR), favorable: false }
  }
  if (airBias === 'boost') {
    return { key: 'wind-boost', label: 'Wind boost', icon: 'Wind', note: windNote(g, 'out'), favorable: true }
  }
  if (airBias === 'drag') {
    return { key: 'wind-drag', label: 'Wind drag', icon: 'Wind', note: windNote(g, 'in'), favorable: false }
  }
  ...
  if (interactionKnown || calm) {
    return { key: 'neutral', label: 'Neutral', icon: 'Minus', note: interactionKnown ? 'Mapped interaction near neutral' : 'Calm air · park near neutral', favorable: false }
  }
  return {
    key: 'unrated',
    label: 'Unrated',
