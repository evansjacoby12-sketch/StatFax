...
// The daily Decision Brief (server-generated by slate-brief.mjs). Accept the
// structured v2 contract plus legacy v1 paragraphs while old caches age out.
// Advisory only: it never blocks the board load.
export function isRenderableBrief(brief) {
  if (!brief || brief.skipped || brief.error) return false
  if (Number(brief.version) >= 2) {
    return Boolean(brief.headline && Array.isArray(brief.leaders))
  }
  return Boolean(brief.text)
}
...
export async function loadBrief() {
  try {
    ...
    if (!isRenderableBrief(b)) return null
    return b
  } catch {
    return null
  }
...
export async function loadSlate() {
  ...
  const batters = uniqueBatters.map((b) => {
    ...
    return {
      ...
      // Versioned, advisory location signal. Legacy relative matches are not
      // promoted into a badge; only a reliable v2 Zone Master qualifies.
      zoneEdge: (b.zoneMatchup?.modelVersion ?? 0) >= 2
        && b.zoneMatchup?.advisoryOnly === true
        && b.zoneMatchup?.reliability?.status !== 'limited'
        && b.zoneMatchup?.badge === 'ZONE_MASTER',
