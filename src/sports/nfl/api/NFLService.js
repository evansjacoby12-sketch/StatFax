...
export function mergeNFLLiveUpdate(snapshot, update) {
  ...
  return {
    ...
    players: snapshot.players.map((player) => {
      ...
      return next ? { ...player, ...next, lineup: { ...player.lineup, ...next.lineup, ...(next.lineup?.marketFactors ? { projectionAdjusted: false } : {}), redZone: { ...player.lineup?.redZone, ...next.lineup?.redZone }, situations: { ...player.lineup?.situations, ...next.lineup?.situations }, marketFactors: { ...player.lineup?.marketFactors, ...next.lineup?.marketFactors } }, live: { ...player.live, ...next.live, participationVerified: next.live?.participationVerified === true, stats: { ...player.live?.stats, ...next.live?.stats } } } : player
    }),
  }
}
