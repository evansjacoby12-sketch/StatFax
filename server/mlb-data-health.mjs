...
try {
  ...
  const result = applyMlbDataHealth({ slate, generatedAt: new Date().toISOString() })
  ...
  console.log(`[mlb-data-health] ${result.report.status}: ${validation.metrics.hardFailures} blocker(s), ${validation.metrics.warnings} warning(s)`)
