    ...
    if (payload.kDistByPitcher && Object.keys(payload.kDistByPitcher).length) {
      ...
      const currentRows = Object.entries(payload.kDistByPitcher).map(([key, kd]) => {
        ...
        return {
          ...
          pitchWhiffMatchup: Number.isFinite(kd.pitchWhiffMatchup) ? +kd.pitchWhiffMatchup.toFixed(1) : null,
          pitchWhiffCoverage: Number.isFinite(kd.pitchWhiffCoverage) ? +kd.pitchWhiffCoverage.toFixed(3) : null,
          pitchWhiffBatters: kd.pitchWhiffBatters ?? null,
          pitchWhiffAdj: Number.isFinite(kd.pitchWhiffAdj) ? +kd.pitchWhiffAdj.toFixed(3) : null,
          h2hKRate: Number.isFinite(kd.h2hKRate) ? +kd.h2hKRate.toFixed(3) : null,
          h2hKAdj: Number.isFinite(kd.h2hKAdj) ? +kd.h2hKAdj.toFixed(3) : null,
          h2hSample: kd.h2hSample ?? null,
          h2hBatters: kd.h2hBatters ?? null,
          matchupAdj: Number.isFinite(kd.matchupAdj) ? +kd.matchupAdj.toFixed(3) : null,
